...
public class ScreenProtector extends javax.swing.JFrame implements Runnable{
	...
	public void run() {
		...
		//Criando loop para criar varios objetos
		/*
		for(int i=0; i<1; i++) {	
		...
		for(Shape s:shapes) {
			if(rand.nextBoolean())
				s.setSpeedX(-(rand.nextInt(2)+1));
			if(rand.nextBoolean())
				s.setSpeedY(-(rand.nextInt()+1));
		}
		*/
		Image logo = new Image("/images/pikachu.png",300,300,"/sounds/impact.wav",false);
		logo.scale(0.5);
